...
class TaroCropperComponent extends React.PureComponent<
  TaroCropperComponentProps,
  TaroCropperComponentState
> {
  static defaultProps = {
    width: 750,
    height: 1200,
  ...
  updateInfo(props: TaroCropperComponentProps) {
    const { width, height, cropperWidth, cropperHeight, src, fullScreen } =
      props;
    this.width = fullScreen
      ? this.systemInfo.windowWidth
      : this._getRealPx(width);
    this.height = fullScreen
      ? this.systemInfo.windowHeight
      : this._getRealPx(height);
    ...
    return Taro.getImageInfo({
      ...
      if (imageWidth / imageHeight < this.cropperWidth / this.cropperHeight) {
        // 宽度充满
        this.scaleImageWidth = this.realImageWidth = this.cropperWidth;
        this.scaleImageHeight = this.realImageHeight =
          (this.realImageWidth * imageHeight) / imageWidth;
        this.imageLeftOrigin = this.imageLeft =
          (this.width - this.cropperWidth) / 2;
        this.imageTopOrigin = this.imageTop =
          (this.height - this.realImageHeight) / 2;
      } else {
        this.scaleImageHeight = this.realImageHeight = this.cropperHeight;
        this.scaleImageWidth = this.realImageWidth =
          (this.realImageHeight * imageWidth) / imageHeight;
        this.imageLeftOrigin = this.imageLeft =
          (this.width - this.realImageWidth) / 2;
        this.imageTopOrigin = this.imageTop =
  ...
  componentDidMount(): void {
    ...
    this.cropperCutCanvasContext = Taro.createCanvasContext(
      cropperCutCanvasId,
      this
    );
  ...
  _drawCropperCorner() {
    ...
    this.cropperCanvasContext.moveTo(
      cropperStartX,
      cropperStartY - lineWidth / 2
    );
    this.cropperCanvasContext.lineTo(cropperStartX, cropperStartY + lineLength);
    // 右上角
    this.cropperCanvasContext.moveTo(
      cropperStartX + this.cropperWidth,
      cropperStartY
    );
    this.cropperCanvasContext.lineTo(
      cropperStartX + this.cropperWidth - lineLength,
      cropperStartY
    );
    this.cropperCanvasContext.moveTo(
      cropperStartX + this.cropperWidth,
    ...
    this.cropperCanvasContext.lineTo(
      cropperStartX + this.cropperWidth,
      cropperStartY + lineLength
    );
    // 左下角
    this.cropperCanvasContext.moveTo(
      cropperStartX,
      cropperStartY + this.cropperHeight
    );
    this.cropperCanvasContext.lineTo(
      cropperStartX + lineLength,
      cropperStartY + this.cropperHeight
    );
    this.cropperCanvasContext.moveTo(
      cropperStartX,
      cropperStartY + this.cropperHeight + lineWidth / 2
    );
    this.cropperCanvasContext.lineTo(
      cropperStartX,
      cropperStartY + this.cropperHeight - lineLength
    );
    // 右下角
    this.cropperCanvasContext.moveTo(
      cropperStartX + this.cropperWidth,
  ...
  _drawCropperContent(
    ...
    const cropperImageX =
      ((cropperStartX - deviationX) / drawWidth) * imageWidth;
    const cropperImageY =
      ((cropperStartY - deviationY) / drawHeight) * imageHeight;
    ...
    // @ts-ignore
    this.cropperCutCanvasContext.drawImage(
      src,
      cropperImageX,
      cropperImageY,
      cropperImageWidth,
  ...
  update() {
    ...
    const src =
      process.env.TARO_ENV === "h5" ? this.image : this.imageInfo.path;
  ...
  componentWillReceiveProps(
    nextProps: Readonly<TaroCropperComponentProps>,
    nextContext: any
  ): void {
    if (JSON.stringify(nextProps) != JSON.stringify(this.props)) {
      this.updateInfo(nextProps).then(() => {
        this.update();
      });
    }
    return (
  ...
  _outsideBound(imageLeft: number, imageTop: number) {
    this.imageLeft =
      imageLeft > (this.width - this.cropperWidth) / 2
        ? (this.width - this.cropperWidth) / 2
        : imageLeft + this.scaleImageWidth >=
          (this.width + this.cropperWidth) / 2
        ? imageLeft
    ...
    this.imageTop =
      imageTop > (this.height - this.cropperHeight) / 2
        ? (this.height - this.cropperHeight) / 2
        : imageTop + this.scaleImageHeight >=
          (this.height + this.cropperHeight) / 2
        ? imageTop
  ...
  _oneTouchMove(touch: CanvasTouch) {
    ...
    this._outsideBound(
      this.imageLeftOrigin + xMove,
      this.imageTopOrigin + yMove
    );
    this.update();
  }
  ...
  _twoTouchMove(touch0: CanvasTouch, touch1: CanvasTouch) {
    ...
    const imageLeft =
      ...
      (this.scaleImageWidth - this.lastScaleImageWidth) / 2;
    const imageTop =
      this.imageTopOrigin -
      (this.scaleImageHeight - this.lastScaleImageHeight) / 2;
  ...
  render(): any {
    ...
    const _width = fullScreen
      ? this.systemInfo.windowWidth
      : this._getRealPx(width);
    const _height = fullScreen
      ? this.systemInfo.windowHeight
      : this._getRealPx(height);
    ...
    if (!hideFinishText) {
      ...
      // if (!isH5) {
      finish = (
        <CoverView style={finishStyle} onClick={onFinishClick}>
          {finishText}
        </CoverView>
      );
      // } else {
    ...
    if (!hideCancelText) {
      ...
      cancel = (
        <CoverView style={cancelStyle} onClick={onCancel}>
          {cancelText}
        </CoverView>
      );
    }
    return (
      <View
        className={`taro-cropper ${
          isFullScreenCss ? "taro-cropper-fullscreen" : ""
        }`}
        style={croperStyle}
      >
        <Canvas
          ...
          className={`cut-canvas-item ${
            isFullScreenCss ? "cut-canvas-fullscreen" : ""
          }`}
        />
        <Canvas
          ...
          className={`canvas-item ${
            isFullScreenCss ? "canvas-fullscreen" : ""
          }`}
          disableScroll
        ></Canvas>
        {!hideFinishText && finish}
        {!hideCancelText && cancel}
      </View>
    );
  }
}
