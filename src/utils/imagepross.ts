...
async function loadImage(imgUrl) {
  ...
  var offscreenCanvas = Taro.createOffscreenCanvas({
    type: "2d",
  });
...
export async function scaleMask(maskFile, model, selectColor, isUp) {
  try {
    ...
    const colorMat = new cv.Mat.ones(
      convertedMask.rows,
      convertedMask.cols,
      cv.CV_8UC4
    );
...
export async function inPaint(imageInfo, maskFile, model, selectColor) {
  try {
    const imageFile = imageInfo.isCropped
      ? imageInfo.croppedCover
      : imageInfo.cover;
    ...
    const mask = convertAndResizeMask(
      maskInput,
      originalImg.width,
      originalImg.height
    );
...
async function mergeCroppedImage(imageInfo, croppedImage) {
  ...
  x_min = Math.min(Math.max(x_min, 0), src.cols);
  let y_min = Math.floor(
    imageInfo.croppedImageData.startY * originalImg.height
  );
  y_min = Math.min(Math.max(y_min, 0), src.rows);
  let width =
    Math.floor(originalImg.width / imageInfo.croppedImageData.scale) - 1;
  let height =
    Math.floor(originalImg.height / imageInfo.croppedImageData.scale) - 1;
...
function saveImageDataToTempFile(image) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const tempFilePath =
      Taro.env.USER_DATA_PATH + "/pic_inpaint_" + number + ".png";
